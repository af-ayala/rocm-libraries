...
Legacy callback functions (deprecated)
--------------------------------------
...
SPIR-V callbacks are preferred over legacy callbacks because they
allow for rocFFT to properly optimize the the combined callback and
FFT code.  Legacy callback functions are already compiled by the time
they are passed to rocFFT, and no further optimization can be done.
